				...
				<div class="content-box">
				  <div class="content-box">
								<div><!-- class="content-left">-->
										<div style="width:100%; height:auto;  min-height:250px;">
										<div style="width: 100%; height: auto;"><br />
											<?php
                                            	$sql_srch="select * from tbltemp where Topic like '%".$_POST['serval']."%'";
												$result_srch=$con->query($sql_srch);
												echo("<table  width='100%' ><tr ><th  align='center'><b>&nbsp;&nbsp;Class</b></th><th align='center'><b>Subject</b></th><th align='center'><b>Topic</b></th><th align='center'><b>Ebook</b></th><th align='center'><b>Video</b></th></tr>");
												if($result_srch->num_rows>0)
												{
													while($rows_srch=$result_srch->fetch_assoc())
													{
														...
														{
															while($rows_getid=$result_getid->fetch_assoc())
															{
																$sql_getextra_details="Select ebook,video from extra_detail where topic_id=".$rows_getid['id'];
																$result_getextradetails=$con->query($sql_getextra_details);
																if($result_getextradetails->num_rows>0)
																{
																	while($rows_getextradetails=$result_getextradetails->fetch_assoc())
																	{
																		$arrebook=explode("/",$rows_getextradetails['ebook']);
																		$arrvideo=explode("/",$rows_getextradetails['video']);
														echo("<tr><td align='center'>&nbsp;&nbsp;".$rows_srch['class']."</td><td align='center'>".$rows_srch['subject']."</td><td align='center'>".$rows_srch['Topic']."</td><td align='center'><a href='admin/".$rows_getextradetails['ebook']."'>".$arrebook[count($arrebook)-1]."</a></td><td align='center'><a href='admin/".$rows_getextradetails['video']."'>".$arrvideo[count($arrvideo)-1]."</a></td></tr>");
																	}
																}
															}
														}
										...
										<!--<div style="width: 25%; float: left; height: auto;">
										    <p id="rank"><?php //echo $res['rank'];?> Rank</p>
										    <p id="rankphoto"><?php //echo $res['name'];?></p>
